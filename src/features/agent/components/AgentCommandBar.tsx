...
export function AgentCommandBar({ leads, onUpdateLead, onParse, isParsing, accessToken, logActivity, isViewer, teamId, userId }: AgentCommandBarProps) {
  ...
  return <section className="w-full max-w-4xl mx-auto px-4 py-6">
    <div className="rounded-2xl border border-[#B74A26]/20 bg-white/75 p-5 shadow-sm backdrop-blur-md">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2"><span className="rounded-xl bg-[#B74A26] p-2 text-[#FDFBF2]"><Bot className="h-5 w-5" /></span><div><p className="text-sm font-bold text-[#1F1612]">Soro Command Center</p><p className="text-[11px] text-[#1F1612]/50">Ask, act, or turn raw context into a lead.</p></div></div>
        <div className="flex items-center gap-2">
          <div className="rounded-lg border border-[#1F1612]/10 bg-[#1F1612]/5 p-0.5 text-xs font-semibold"><button type="button" onClick={() => switchMode("ask")} className={`rounded-md px-3 py-1.5 ${mode === "ask" ? "bg-white text-[#1F1612] shadow-sm" : "text-[#1F1612]/55"}`}>Ask Soro</button><button type="button" onClick={() => switchMode("capture")} className={`rounded-md px-3 py-1.5 ${mode === "capture" ? "bg-white text-[#1F1612] shadow-sm" : "text-[#1F1612]/55"}`}>Capture lead</button></div>
          {!isViewer && <button type="button" onClick={openKnowledge} className="flex items-center gap-1 rounded-lg border border-[#1F1612]/10 bg-[#1F1612]/5 px-2.5 py-1.5 text-[11px] font-bold text-[#1F1612]/70 hover:bg-[#1F1612]/10"><BookOpen className="h-3.5 w-3.5" />Knowledge</button>}
        </div>
      </div>
      {threads.length > 0 && <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        {threads.map((t) => <button key={t.id} type="button" onClick={() => switchThread(t)} className={`rounded-lg border px-2.5 py-1 text-[11px] font-semibold truncate max-w-[160px] ${threadId === t.threadId ? "border-[#B74A26]/40 bg-[#B74A26]/10 text-[#B74A26]" : "border-[#1F1612]/10 bg-white text-[#1F1612]/70 hover:bg-[#1F1612]/5"}`}>{t.title || "Conversation"}</button>)}
      </div>}
      <form onSubmit={submit}>
        <textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} disabled={isWorking} placeholder={mode === "ask" ? (threadId ? "Continue the conversation..." : "Ask Soro anything — follow up with Sarah next Tuesday") : "Paste a LinkedIn bio, email signature, meeting note, or transcript"} className="min-h-28 w-full resize-none rounded-xl border border-[#1F1612]/10 bg-[#FDFBF2]/60 px-4 py-3 text-sm text-[#1F1612] outline-none placeholder:text-[#1F1612]/40 focus:border-[#B74A26]/40" />
        <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
          {mode === "capture" ? <div className="flex flex-wrap items-center gap-2"><div className="rounded-lg border border-[#1F1612]/10 bg-[#1F1612]/5 p-0.5 text-[11px] font-bold"><button type="button" onClick={() => setModelPreset("low-latency")} className={`rounded-md px-2 py-1 ${modelPreset === "low-latency" ? "bg-white shadow-sm" : "text-[#1F1612]/50"}`}>Lite</button><button type="button" onClick={() => setModelPreset("high-quality")} className={`rounded-md px-2 py-1 ${modelPreset === "high-quality" ? "bg-white shadow-sm" : "text-[#1F1612]/50"}`}>Balanced</button><button type="button" onClick={() => setModelPreset("deep-reasoning")} className={`rounded-md px-2 py-1 ${modelPreset === "deep-reasoning" ? "bg-white shadow-sm" : "text-[#1F1612]/50"}`}>Reason</button></div><button type="button" onClick={() => setSearchGrounding((active) => !active)} className={`flex items-center gap-1 rounded-lg border px-2.5 py-1.5 text-[11px] font-bold ${searchGrounding ? "border-[#7A8452]/40 bg-[#7A8452]/10 text-[#7A8452]" : "border-[#1F1612]/10 text-[#1F1612]/55"}`}><Globe className="h-3.5 w-3.5" />Web research</button></div> : <p className="text-[11px] text-[#1F1612]/50">Search leads · Write email · Schedule meetings · Update pipeline</p>}
          <button type="submit" disabled={!prompt.trim() || isWorking} className="flex items-center gap-2 rounded-xl bg-[#B74A26] px-4 py-2 text-xs font-bold text-[#FDFBF2] disabled:opacity-40">{isWorking ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <ArrowUp className="h-4 w-4" />}{isWorking ? "Soro is working…" : mode === "ask" ? "Ask Soro" : "Create lead"}</button>
        </div>
      </form>
      {plan && <div className="mt-4 border-t border-[#1F1612]/10 pt-4"><div className="flex gap-2 text-sm leading-relaxed text-[#1F1612]"><Sparkles className="mt-0.5 h-4 w-4 shrink-0 text-[#B74A26]" /><p>{plan.response}</p></div>{plan.actions.length > 0 && <div className="mt-3 space-y-2">{plan.actions.map((action, index) => <div key={`${action.type}-${index}`} className="flex items-center gap-2 rounded-lg bg-[#FDFBF2] px-3 py-2 text-xs text-[#1F1612]/75">{action.type === "send_email" ? <Mail className="h-3.5 w-3.5 text-[#B74A26]" /> : action.type === "schedule_meeting" ? <CalendarDays className="h-3.5 w-3.5 text-[#CFA331]" /> : <Check className="h-3.5 w-3.5 text-[#7A8452]" />}<span>{ACTION_LABELS[action.type]}{action.leadId ? ` · ${getLead(action.leadId)?.name || action.leadId}` : action.name ? ` · ${action.name}` : ""}</span></div>)}<button type="button" onClick={applyPlan} disabled={isViewer || isApplying} className="mt-2 rounded-xl bg-[#B74A26] px-4 py-2 text-xs font-bold text-[#FDFBF2] disabled:cursor-not-allowed disabled:opacity-40">{isApplying ? "Soro is working…" : hasExternalActions ? "Confirm and apply plan" : "Apply plan"}</button>{hasExternalActions && <p className="text-[11px] text-[#1F1612]/45">Email and calendar actions run only after you confirm.</p>}</div>}</div>}
    </div>
    {isKnowledgeOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-lg rounded-2xl border border-[#1F1612]/10 bg-white p-6 shadow-xl">
        ...
        <p className="text-[11px] text-[#1F1612]/55 mb-4">This context is injected into every Soro request so the agent understands how your team operates.</p>
